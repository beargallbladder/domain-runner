...
exports.IntelligenceGraphSystem = void 0;
class IntelligenceGraphSystem {
    constructor(pool, logger, memoryOracle) {
        this.nodeCache = new Map();
        this.edgeCache = new Map();
        this.clusterCache = new Map();
        // Mapping functions
        this.mapToIntelligenceNode = (row) => ({
            id: row.id,
        ...
        this.mapToIntelligenceEdge = (row) => ({
            id: row.id,
    ...
    `;
        ...
        for (const row of result.rows) {
            ...
            await this.createOrUpdateEdge(row.source_node_id, row.target_node_id, 'competitor', row.strength, row.confidence, 'directed', properties);
        }
        this.logger.info(`🤝 Created competitive edges`);
    }
    // Create influence edges from patterns to domains
    async createInfluenceEdges() {
        const query = `
    ...
    `;
        ...
        `;
                ...
                if (domainResult.rows.length > 0) {
                    ...
                    await this.createOrUpdateEdge(row.pattern_node_id, domainResult.rows[0].id, 'influences', row.effectiveness, row.confidence, 'directed', properties);
                }
            }
        }
    ...
    `;
        ...
        for (const row of result.rows) {
            ...
            await this.createOrUpdateEdge(row.node1_id, row.node2_id, 'correlates', correlationStrength, row.avg_confidence, 'undirected', properties);
        }
        this.logger.info(`🔗 Created correlation edges`);
    }
    // Create prediction edges from predictions to targets
    async createPredictionEdges() {
        const query = `
      SELECT pm.*, 
    ...
    `;
        ...
        for (const row of result.rows) {
            ...
            await this.createOrUpdateEdge(row.prediction_node_id, row.domain_node_id, 'predicts', row.confidence, row.confidence, 'directed', properties);
        }
    ...
    async findStrategicPaths(sourceId, targetId, pathType) {
        try {
    ...
    async getCompetitiveIntelligence(domainId) {
        try {
    ...
    async createOrUpdateNode(nodeType, entityId, label, properties, confidence, importance) {
        const query = `
    ...
    async createOrUpdateEdge(sourceId, targetId, edgeType, strength, confidence, direction, properties) {
        const query = `
    ...
    calculateDomainImportance(domainData) {
        let importance = 0.5;
        if (domainData.is_jolt)
            importance += 0.3;
        if (domainData.response_count > 10)
            importance += 0.2;
        if (domainData.completeness > 0.8)
            importance += 0.2;
        return Math.min(importance, 1.0);
    }
    calculateSynthesisImportance(synthesis) {
    ...
    dijkstra(adjacencyList, start, end) {
    ...
    calculatePathConfidence(path, edges) {
        // Calculate aggregate confidence along path
        return 0.8;
    }
    async getDirectCompetitors(domainId) {
        const query = `
    ...
    async getInfluencingPatterns(domainId) {
        const query = `
      SELECT n.* FROM intelligence_nodes n
    ...
    async getRelevantPredictions(domainId) {
        const query = `
      SELECT n.* FROM intelligence_nodes n
      JOIN intelligence_edges e ON n.id = e.source_id
    ...
    async getClusterMembership(domainId) {
        const query = `
...
exports.IntelligenceGraphSystem = IntelligenceGraphSystem;
