...
class CorrelationEngine {
            ...
            try {
                ...
                for (const event of recentEvents) {
                    ...
                    for (const correlation of correlations) {
                        await database_1.db.storeCorrelation(correlation);
                        correlationsCreated++;
                    }
                    eventsProcessed++;
                }
    ...
    async getRecentEvents(domain, days) {
        try {
            if (!domain || days <= 0) {
                return [];
            }
    ...
    async findPerceptionCorrelations(event) {
        const correlations = [];
        ...
        const beforeData = await database_1.db.getRecentPerceptionData(event.domain, beforeDate);
        const afterData = await database_1.db.getRecentPerceptionData(event.domain, eventDate);
    ...
    calculateModelScores(beforeData, afterData, eventDate) {
        const modelScores = {};
        // Get unique models
    ...
    calculatePerceptionScore(responses) {
    ...
    calculateCorrelationStrength(beforeScore, afterScore, event) {
