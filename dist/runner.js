...
async function processDomain(domain) {
    try {
        ...
        const domainId = await (0, db_1.saveDomain)(domain);
        console.log(`✅ Domain saved: ${domain} (ID: ${domainId})`);
        // Process each prompt
        for (const prompt of prompts_1.TRIAD_001_PROMPTS) {
            try {
                // Get response from OpenAI
                const llmResponse = await (0, openai_1.getOpenAIResponse)(domain, prompt);
                // Save the response
                await (0, db_1.saveResponse)({
                    domain_id: domainId,
