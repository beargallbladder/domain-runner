...
const fs = require('fs');
const { testAllProviders } = require('./llm-status-checker');
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      '/api/trigger-crawl',
      '/api/crawl-status',
      '/api/llm-status' // NEW! Comprehensive LLM status
    ]
  });
});
...
app.post('/api/trigger-crawl', auth, async (req, res) => {
  try {
    ...
    // Try simple test crawler first to debug
    const crawlerFile = fs.existsSync(__dirname + '/crawler-tensor.js') 
      ? 'crawler-tensor.js' 
      : 'simple-test-crawl.js';
    
    console.log(`Starting crawler: ${crawlerFile}`);
    
    const crawler = spawn('node', [crawlerFile], {
      cwd: __dirname,
      detached: true,
      stdio: 'ignore',
      env: process.env
...
// 🔬 NEW: LLM STATUS CHECKER ENDPOINT
app.get('/api/llm-status', auth, async (req, res) => {
  try {
    console.log('Running comprehensive LLM status check...');
    const results = await testAllProviders();
    
    // Format response
    const response = {
      timestamp: new Date().toISOString(),
      environment: process.env.RENDER ? 'RENDER_PRODUCTION' : 'LOCAL',
      summary: {
        total_providers: results.working.length + results.failed.length + results.no_key.length,
        working: results.working.length,
        failed: results.failed.length,
        missing_keys: results.no_key.length,
        success_rate: Math.round((results.working.length / (results.working.length + results.failed.length + results.no_key.length)) * 100) + '%'
      },
      working_providers: results.working.map(p => ({
        name: p.name,
        response_time_ms: p.responseTime,
        sample: p.sampleResponse
      })),
      failed_providers: results.failed.map(p => ({
        name: p.name,
        error: p.error
      })),
      missing_api_keys: results.no_key
    };
    ...
    res.status(500).json({ 
      error: error.message,
      message: 'Failed to run LLM status check'
    });
  }
});
