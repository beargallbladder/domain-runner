...
export class NewsScanner {
  ...
  async scanGoogleNews(domains: string[]): Promise<NewsEvent[]> {
  ...
  private isRelevantCrisisNews(headline: string, domain: string): boolean {
    ...
    const containsCrisisKeyword = this.crisisKeywords.some(keyword => 
  ...
  private classifyEventType(headline: string): string {
    const lowerHeadline = headline.toLowerCase();
  ...
  async scanAllSources(domains: string[]): Promise<ScanResult> {
    ...
    const uniqueEvents = allEvents.filter((event, index, array) => 
      array.findIndex(e => e.headline === event.headline && e.domain === event.domain) === index
    );
