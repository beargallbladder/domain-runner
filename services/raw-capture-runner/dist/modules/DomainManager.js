...
class DomainManager {
     ...
     */
    async addDomains(request) {
        const { domains, cohort = 'manual', source = 'api_injection', priority = 1 } = request;
     ...
     */
    async updateDomainCohort(domains, newCohort) {
        const client = await this.primaryPool.connect();
        try {
