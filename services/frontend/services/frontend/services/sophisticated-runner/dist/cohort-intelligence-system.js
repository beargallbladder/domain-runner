...
exports.CohortIntelligenceSystem = void 0;
...
class CohortIntelligenceSystem {
    ...
    // ============================================================================
    async generateComprehensiveCohorts() {
    ...
    async buildCohortMembers(domains, cohortConfig) {
        const members = [];
    ...
    isDomainInCohort(domain, cohortConfig) {
    ...
    calculateCompetitivePosition(member, allMembers) {
    ...
    async analyzeCohort(cohortName, cohortConfig, members) {
    ...
    async generateCompetitiveNarrative(cohortName, leader, laggard, members) {
    ...
    // ============================================================================
    async discoverMissingCompetitors(cohortConfig) {
    ...
    extractDomainsFromResponse(response) {
        const domains = [];
    ...
    async queueDomainsForProcessing(domains, cohortName) {
        const client = await this.pool.connect();
        try {
            for (const domain of domains) {
                // Check if domain already exists
                const existingResult = await client.query('SELECT domain FROM public_domain_cache WHERE domain = $1', [domain]);
    ...
    async generateCohortAPI() {
        ...
        for (const [cohortName, analysis] of Object.entries(cohorts)) {
            const category = {
                ...
                topDomains: JSON.stringify(analysis.members.slice(0, 5)),
        ...
        apiResponse.categories.sort((a, b) => {
...
exports.CohortIntelligenceSystem = CohortIntelligenceSystem;
// Export for integration with sophisticated runner
exports.default = CohortIntelligenceSystem;
//# sourceMappingURL=cohort-intelligence-system.js.map